...
class TestReporter {
  ...
  generateMarkdownReport(testSummary, coverageSummary) {
    ...
    if (testSummary && testSummary.success && coverageSummary && coverageSummary.overallPassed) {
  ...
  generateJsonReport(testSummary, coverageSummary) {
    return {
      ...
      overall: {
        success: (testSummary?.success ?? true) && (coverageSummary?.overallPassed ?? true)
      }
    };
  }
  ...
  async generateReports() {
    ...
    const markdownReport = this.generateMarkdownReport(testSummary, coverageSummary);
  ...
  generateBadgeData(testSummary, coverageSummary) {
    ...
    if (coverageSummary) {
      ...
      badges.coverage = {
        ...
        label: 'coverage',
        message: `${avgCoverage}%`,
        color: avgCoverage >= 90 ? 'brightgreen' : avgCoverage >= 80 ? 'yellow' : 'red'
      };
    }
  ...
  printConsoleSummary(testSummary, coverageSummary) {
    ...
    if (testSummary) {
      ...
      console.log(`📁 Suites: ${testSummary.passedSuites}/${testSummary.testSuites} passed`);
    }
    
    if (coverageSummary) {
      console.log(`\n📈 Coverage:`);
      console.log(`   Statements: ${coverageSummary.statements.pct.toFixed(2)}% (${coverageSummary.statements.passed ? '✅' : '❌'})`);
      console.log(`   Branches:   ${coverageSummary.branches.pct.toFixed(2)}% (${coverageSummary.branches.passed ? '✅' : '❌'})`);
      console.log(`   Functions:  ${coverageSummary.functions.pct.toFixed(2)}% (${coverageSummary.functions.passed ? '✅' : '❌'})`);
      console.log(`   Lines:      ${coverageSummary.lines.pct.toFixed(2)}% (${coverageSummary.lines.passed ? '✅' : '❌'})`);
    }
    
    const overallSuccess = (testSummary?.success ?? true) && (coverageSummary?.overallPassed ?? true);
...
async function main() {
  ...
  const options = {
    ...
    coverageDir: args.find(arg => arg.startsWith('--coverage-dir='))?.split('=')[1],
