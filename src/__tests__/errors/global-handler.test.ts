...
describe('GlobalErrorHandler', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle error reporter failures', () => {
      ...
      const log = handler.getErrorLog();
      expect(log.some(entry => entry.message.includes('Error reporter failed'))).toBe(true);
    });
  });
  ...
  describe('Error Statistics', () => {
    ...
    it('should provide error statistics', () => {
      ...
      handler.handleError(new TasklyError('Error 2', ERROR_CODES.VALIDATION_ERROR));
      handler.handleError(new TasklyError('Error 3', ERROR_CODES.SECURITY_VIOLATION));
      handler.log(LogLevel.WARN, 'Warning message');
      handler.log(LogLevel.INFO, 'Info message');
    ...
    it('should limit recent errors to 10', () => {
      ...
      for (let i = 0; i < 15; i++) {
        handler.handleError(new TasklyError(`Error ${i}`, ERROR_CODES.TASK_FAILED));
      }
  ...
  describe('Shutdown Callbacks', () => {
    ...
    it('should add and execute shutdown callbacks', async () => {
      ...
      const gracefulShutdownSpy = vi.spyOn(handler, 'gracefulShutdown').mockImplementation(async () => {
    ...
    it('should handle callback errors during shutdown', async () => {
      ...
      const executeCallbacksSpy = vi.spyOn(handler as any, 'executeShutdownCallbacks')
