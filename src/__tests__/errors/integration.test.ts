...
describe('Error Handling Integration Tests', () => {
  ...
  describe('TaskRunner Error Integration', () => {
    it('should handle validation errors during task execution', async () => {
      ...
      await expect(taskRunner.execute(invalidTasks)).rejects.toThrow(TasklyError);
      
      // Check that error was logged globally
      const errorLog = globalHandler.getErrorLog();
      expect(errorLog.length).toBeGreaterThan(0);
      
      const validationErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.VALIDATION_ERROR
      );
    ...
    it('should handle duplicate task identifier errors', async () => {
      ...
      await expect(taskRunner.execute(duplicateTasks)).rejects.toThrow(TasklyError);
    ...
    it('should handle concurrent execution prevention', async () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const systemErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.SYSTEM_ERROR
      );
  ...
  describe('ProcessManager Error Integration', () => {
    ...
    it('should handle security violations in command validation', async () => {
      ...
      await expect(processManager.spawn(dangerousTask)).rejects.toThrow(SecurityError);
      
      const errorLog = globalHandler.getErrorLog();
      const securityErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.COMMAND_INJECTION
      );
    ...
    it('should handle spawn failures with proper error context', async () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const spawnErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.SPAWN_FAILED
      );
      ...
      expect(spawnError.error?.context.command).toBe('nonexistent-command-12345');
    });
    ...
    it('should handle multiple security violations', async () => {
      ...
      for (const command of dangerousTasks) {
        const task: TaskConfig = { command, identifier: `dangerous-${command.slice(0, 5)}` };
  ...
  describe('Error Recovery and Retry Logic', () => {
    it('should handle recoverable errors appropriately', () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const timeoutErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.PROCESS_TIMEOUT
      );
    ...
    it('should handle non-recoverable errors appropriately', () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const securityErrors = errorLog.filter(entry => 
        entry.error?.code === ERROR_CODES.SECURITY_VIOLATION
      );
  ...
  describe('Error Context Propagation', () => {
    it('should propagate error context through the system', async () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const contextErrors = errorLog.filter(entry => 
        entry.error?.context.taskId === 'context-task'
      );
    ...
    it('should handle nested error contexts', () => {
      ...
      const errorLog = globalHandler.getErrorLog();
      const nestedErrors = errorLog.filter(entry => 
        entry.error?.context.taskId === 'nested-task'
      );
      ...
      expect(error.error?.context.metadata).toHaveProperty('originalMessage', 'Original system error');
      expect(error.context?.metadata).toHaveProperty('testCase', 'nested-context');
    });
  });
  ...
  describe('Error Aggregation and Reporting', () => {
    it('should aggregate errors from multiple sources', () => {
      const errors = [
        ErrorFactory.createError('Task error 1', ERROR_CODES.TASK_FAILED, { taskId: 'task-1' }),
        ErrorFactory.createError('Validation error', ERROR_CODES.VALIDATION_ERROR, { taskId: 'task-2' }),
        ErrorFactory.createError('Security error', ERROR_CODES.SECURITY_VIOLATION, { taskId: 'task-3' }),
        ErrorFactory.createError('System error', ERROR_CODES.SYSTEM_ERROR, { taskId: 'task-4' })
      ];
      ...
      expect(stats.recentErrors.every(entry => entry.context?.batch === 'aggregation-test')).toBe(true);
    });

    it('should provide comprehensive error reporting', () => {
      // Simulate various error scenarios
      const scenarios = [
        { error: ERROR_CODES.PM_NOT_FOUND, context: { packageManager: 'yarn' } },
        { error: ERROR_CODES.SPAWN_FAILED, context: { command: 'failed-command' } },
        { error: ERROR_CODES.PROCESS_TIMEOUT, context: { timeout: 30000 } },
        { error: ERROR_CODES.PERMISSION_DENIED, context: { path: '/restricted/path' } },
        { error: ERROR_CODES.RESOURCE_EXHAUSTED, context: { resource: 'memory' } }
      ];
      ...
      expect(errorLog.every(entry => entry.context?.testType === 'comprehensive-reporting')).toBe(true);
      
      // Verify different error severities are represented
      const severityCounts = Object.values(stats.errorsBySeverity);
      const totalSeverityErrors = severityCounts.reduce((sum, count) => sum + count, 0);
  ...
  describe('Shutdown and Cleanup Integration', () => {
    it('should handle shutdown callbacks from multiple components', async () => {
      ...
      const gracefulShutdownSpy = vi.spyOn(globalHandler, 'gracefulShutdown')
    ...
    it('should handle cleanup errors gracefully', async () => {
      ...
      const executeCallbacksSpy = vi.spyOn(globalHandler as any, 'executeShutdownCallbacks')
        .mockImplementation(async () => {
          await Promise.all([
