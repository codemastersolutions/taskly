...
describe('TasklyCLI Integration', () => {
  ...
  describe('help and version', () => {
    it('should display help when --help is provided', async () => {
      ...
      expect(mockConsole.log).toHaveBeenCalledWith(
        expect.stringContaining('Taskly - Zero-dependency parallel command execution')
      );
      expect(mockConsole.log).toHaveBeenCalledWith(
  ...
  describe('error handling', () => {
    ...
    it('should handle unexpected errors', async () => {
      ...
      cli['createTaskConfigs'] = vi.fn().mockRejectedValue(new Error('Unexpected error'));
  ...
  describe('configuration loading', () => {
    it('should load and use JSON configuration', async () => {
      ...
      await expect(cli.run(['--config', configPath, 'echo test'])).resolves.not.toThrow();
    });
...
`;
  ...
  describe('task creation from config', () => {
    ...
    it('should throw error when no tasks available', async () => {
      await expect(cli['createTaskConfigs']({ commands: [] }, null))
  ...
  describe('identifier generation', () => {
    ...
    it('should handle commands with special characters', () => {
      expect(cli['generateIdentifier']('node --max-old-space-size=4096 build.js', 0))
        .toBe('node-0');
      expect(cli['generateIdentifier']('docker-compose up -d', 1))
        .toBe('dockercompose-1');
    });
  });
  ...
  describe('Real Command Execution', () => {
    ...
    it('should handle kill-others-on-fail functionality', async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.find(r => r.identifier === 'failing-task')?.exitCode).toBe(1);
      // The long task should be killed (exit code 130 for SIGINT)
      expect(results.find(r => r.identifier === 'long-task')?.exitCode).toBeGreaterThan(0);
    });
  ...
  describe('Error Recovery and Resilience', () => {
    it('should handle malformed commands gracefully', async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.find(r => r.identifier === 'bad-command')?.exitCode).toBeGreaterThan(0);
      expect(results.find(r => r.identifier === 'good-command')?.exitCode).toBe(0);
    });
